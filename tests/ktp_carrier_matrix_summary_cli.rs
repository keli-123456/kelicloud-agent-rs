...
#[test]
fn ktp_carrier_matrix_summary_rejects_earlier_batch_row_without_reuse() {
    let csv_path = write_temp_csv(
        "ktp-carrier-matrix-summary-earlier-no-reuse",
        r#"carrier,crypto,direction,runs,frames,payload_bytes,write_batch_frames,write_batch_reused,read_batch_frames,read_batch_reused,elapsed_ms_min,elapsed_ms_median,elapsed_ms_max,throughput_mib_s_min,throughput_mib_s_median,throughput_mib_s_max
ktp_tcp,ktp_aead,client_to_relay,3,256,1024,0,0,0,0,20.0,21.0,22.0,11.0,12.0,13.0
ktp_tcp,ktp_aead,client_to_relay_batch_write,3,512,1024,64,0,0,0,10.0,11.0,12.0,21.0,22.0,23.0
ktp_tcp,ktp_aead,client_to_relay_batch_write,3,512,4096,64,1,0,0,10.0,11.0,12.0,31.0,32.0,33.0
ktp_tcp,ktp_aead,relay_to_client_batch_read,3,256,1024,0,0,64,1,9.0,10.0,11.0,41.0,42.0,43.0
"#,
    );
    ...
    assert_eq!(
        output.status.code(),
        Some(3),
        "earlier missing reuse should exit 3: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains(
        "client_to_relay_batch_write frames=512 payload_bytes=1024 write_batch_reused is not 1"
    ));
}

#[test]
fn ktp_carrier_matrix_summary_reports_min_throughput_across_all_shapes() {
    let csv_path = write_temp_csv(
        "ktp-carrier-matrix-summary-min-throughput",
        r#"carrier,crypto,direction,runs,frames,payload_bytes,write_batch_frames,write_batch_reused,read_batch_frames,read_batch_reused,elapsed_ms_min,elapsed_ms_median,elapsed_ms_max,throughput_mib_s_min,throughput_mib_s_median,throughput_mib_s_max
ktp_tcp,ktp_aead,client_to_relay,3,256,1024,0,0,0,0,20.0,21.0,22.0,11.0,12.0,13.0
ktp_tcp,ktp_aead,client_to_relay,3,512,4096,0,0,0,0,20.0,21.0,22.0,41.0,42.0,43.0
ktp_tcp,ktp_aead,client_to_relay_batch_write,3,256,1024,64,1,0,0,10.0,11.0,12.0,21.0,22.0,23.0
ktp_tcp,ktp_aead,client_to_relay_batch_write,3,512,4096,64,1,0,0,10.0,11.0,12.0,51.0,52.0,53.0
ktp_tcp,ktp_aead,relay_to_client_batch_read,3,256,1024,0,0,64,1,9.0,10.0,11.0,31.0,32.0,33.0
ktp_tcp,ktp_aead,relay_to_client_batch_read,3,512,4096,0,0,64,1,9.0,10.0,11.0,61.0,62.0,63.0
"#,
    );
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("ktp_carrier_matrix_summary rows=6 gate=pass"));
    assert!(stdout.contains("client_to_relay_min_throughput_mib_s_median=12.000"));
    assert!(stdout.contains("batch_write_min_throughput_mib_s_median=22.000"));
    assert!(stdout.contains("batch_read_min_throughput_mib_s_median=32.000"));
}
...
"#,
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains(
        "client_to_relay_batch_write frames=256 payload_bytes=1024 write_batch_reused is not 1"
    ));
}
...
"#,
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains(
        "client_to_relay_batch_write frames=256 payload_bytes=1024 throughput_mib_s_median must be positive"
    ));
}
...
"#,
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains(
        "client_to_relay_batch_write frames=256 payload_bytes=1024 throughput_mib_s_median 22.000 below min 30.000"
    ));
}
