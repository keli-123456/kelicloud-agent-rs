...
fixed	1	8	50000	250000	16	8	8	rdp-like	8192	pass	123	logs/fixed/clients-1	logs/fixed/clients-1/tunnel-echo.evidence.md	logs/fixed/clients-1/ktp-live-canary.evidence.md	9920	100000	100	200	300	400	0	3	40	2	2	40	5	64	64	64	0	0
adaptive	4	4	40000	120000	24	6	8	rdp-like	8192	pass	456	logs/adaptive/clients-4	logs/adaptive/clients-4/tunnel-echo.evidence.md	logs/adaptive/clients-4/ktp-live-canary.evidence.md	39680	200000	500	600	700	800	90	12	224	11	10	236	12	64	16	16	1	2
"#,
    );
    ...
    assert!(stdout.contains("policy=fixed clients=1 status=pass elapsed_millis=123 total_payload_bytes=9920 throughput_mib_s=0.077 echo_elapsed_micros=100000 echo_throughput_mib_s=0.095 rtt_micros_p95=200 rtt_client_p95_spread_micros=0 socket_read_max_batch_frames=2 socket_write_max_batch_frames=5 socket_write_batch_limit_max=64 socket_write_batch_limit_min=64 socket_write_batch_limit_last=64 backend_session_limit_count=0 backend_session_not_found_count=0"));
    assert!(stdout.contains("policy=adaptive clients=4 status=pass elapsed_millis=456 total_payload_bytes=39680 throughput_mib_s=0.083 echo_elapsed_micros=200000 echo_throughput_mib_s=0.189 rtt_micros_p95=600 rtt_client_p95_spread_micros=90 socket_read_max_batch_frames=11 socket_write_max_batch_frames=12 socket_write_batch_limit_max=64 socket_write_batch_limit_min=16 socket_write_batch_limit_last=16 backend_session_limit_count=1 backend_session_not_found_count=2"));
    ...
    assert!(stdout.contains("max_backend_session_limit_count=1 policy=adaptive clients=4"));
    assert!(stdout.contains("max_backend_session_not_found_count=2 policy=adaptive clients=4"));
}
...
#[test]
fn ktp_tunnel_matrix_summary_separates_policy_comparisons_by_auth_version() {
    let summary_path = write_temp_summary(
        "ktp-tunnel-matrix-summary-auth-version",
        r#"relay_batch_policy	ktp_auth_version	clients	rounds	profile	payload_bytes	status	elapsed_millis	log_dir	tunnel_evidence_file	ktp_evidence_file	total_payload_bytes	rtt_micros_p50	rtt_micros_p95	rtt_micros_p99	rtt_micros_max	rtt_client_p95_spread_micros	socket_read_batches	socket_read_frames	socket_read_max_batch_frames
fixed	v1	2	8	rdp-like	8192	pass	500	logs/fixed/clients-2	logs/fixed/clients-2/tunnel-echo.evidence.md	logs/fixed/clients-2/ktp-live-canary.evidence.md	19840	700	1000	1200	1500	200	12	112	6
adaptive	v1	2	8	rdp-like	8192	pass	450	logs/adaptive/clients-2	logs/adaptive/clients-2/tunnel-echo.evidence.md	logs/adaptive/clients-2/ktp-live-canary.evidence.md	19840	500	800	900	1000	100	14	112	8
fixed	v2	2	8	rdp-like	8192	pass	480	logs/fixed/v2/clients-2	logs/fixed/v2/clients-2/tunnel-echo.evidence.md	logs/fixed/v2/clients-2/ktp-live-canary.evidence.md	19840	700	900	1200	1500	120	12	112	6
adaptive	v2	2	8	rdp-like	8192	pass	510	logs/adaptive/v2/clients-2	logs/adaptive/v2/clients-2/tunnel-echo.evidence.md	logs/adaptive/v2/clients-2/ktp-live-canary.evidence.md	19840	500	850	1000	1200	110	14	112	8
"#,
    );
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("policy=fixed clients=2 status=pass elapsed_millis=480"));
    assert!(stdout.contains("auth_version=v2"));
    assert!(stdout
        .contains("policy_compare clients=2 fixed_elapsed_millis=500 adaptive_elapsed_millis=450"));
    assert!(stdout
        .contains("policy_compare clients=2 fixed_elapsed_millis=500 adaptive_elapsed_millis=450"));
    assert!(stdout.contains("verdict=adaptive_better auth_version=v1"));
    assert!(stdout.contains(
        "policy_recommend clients=2 recommended=adaptive verdict=adaptive_better reason=adaptive_not_worse auth_version=v1"
    ));
    assert!(stdout
        .contains("policy_compare clients=2 fixed_elapsed_millis=480 adaptive_elapsed_millis=510"));
    assert!(stdout.contains("verdict=mixed auth_version=v2"));
    assert!(stdout.contains(
        "policy_recommend clients=2 recommended=manual_review verdict=mixed reason=metric_tradeoff auth_version=v2"
    ));
    assert!(stdout.contains(
        "expected_matrix policies=fixed,adaptive auth_versions=v1,v2 clients=2 status=pass missing=0"
    ));
}
...
#[test]
fn ktp_tunnel_matrix_summary_relay_error_gates_reject_backend_errors() {
    let summary_path = write_temp_summary(
        "ktp-tunnel-matrix-summary-relay-error-gate",
        r#"relay_batch_policy	clients	rounds	profile	payload_bytes	status	elapsed_millis	log_dir	tunnel_evidence_file	ktp_evidence_file	total_payload_bytes	echo_elapsed_micros	rtt_micros_p50	rtt_micros_p95	rtt_micros_p99	rtt_micros_max	rtt_client_p95_spread_micros	socket_read_batches	socket_read_frames	socket_read_max_batch_frames	backend_session_limit_count	backend_session_not_found_count
adaptive	8	8	rdp-like	8192	pass	456	logs/adaptive/clients-8	logs/adaptive/clients-8/tunnel-echo.evidence.md	logs/adaptive/clients-8/ktp-live-canary.evidence.md	79360	29299749	100	20000	24000	30000	12000	24	640	16	0	1
"#,
    );

    let output = Command::new(summary_exe())
        .arg("--max-backend-session-limit-count")
        .arg("0")
        .arg("--max-backend-session-not-found-count")
        .arg("0")
        .arg(&summary_path)
        .output()
        .expect("ktp-tunnel-matrix-summary should run");
    ...
    assert_eq!(
        output.status.code(),
        Some(3),
        "relay error gate should exit 3: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains(
        "tunnel matrix row policy=adaptive clients=8 backend_session_not_found_count=1 exceeds max 0"
    ));
}
