...
#[test]
fn ktp_policy_summary_cli_recommends_policy_for_each_verdict() {
    let csv_path = write_temp_csv(
        "ktp-policy-summary-recommend",
        r#"profile,runs,clients,frames,payload_bytes,relay_batch_frames,relay_batch_policy,relay_batch_frames_effective,elapsed_ms_min,elapsed_ms_median,elapsed_ms_max,throughput_mib_s_min,throughput_mib_s_median,throughput_mib_s_max,rtt_micros_p50,rtt_micros_p95,rtt_micros_p99,rtt_micros_max,rtt_client_p95_micros_min,rtt_client_p95_micros_max,rtt_client_p95_spread_micros,rtt_client_max_micros_max,relay_turns,relay_wait_turns,ingress_batches,egress_batches,ingress_max_batch_frames,egress_max_batch_frames
rdp-like,5,2,64,8192,64,fixed,64,20.000,25.000,30.000,8.000,10.000,12.000,200,1000,2000,4000,800,1200,400,4000,100,80,60,60,8,8
rdp-like,5,2,64,8192,64,adaptive,16,18.000,23.000,28.000,9.000,11.000,13.000,180,800,1700,3000,700,1000,200,3000,120,90,70,70,6,6
rdp-like,5,4,64,8192,64,fixed,64,19.807,27.361,34.914,6.755,9.331,11.907,227,949,2940,5768,858,1106,248,5768,784,609,392,392,4,4
rdp-like,5,4,64,8192,64,adaptive,32,34.063,44.550,55.037,4.285,5.604,6.924,228,1456,3654,8002,703,2761,2058,8002,856,754,425,397,4,4
rdp-like,5,6,64,8192,64,fixed,64,16.731,24.788,28.730,8.209,9.514,14.096,216,891,2156,3575,841,922,81,3575,1848,1434,950,957,7,4
rdp-like,5,6,64,8192,64,adaptive,64,20.894,27.983,38.629,6.105,8.428,11.287,264,954,2520,4504,839,1087,248,4504,1812,1377,951,956,6,5
rdp-like,5,8,64,8192,64,fixed,64,20.000,25.000,30.000,8.000,10.000,12.000,200,800,2000,4000,800,1200,400,4000,100,80,60,60,8,8
rdp-like,5,8,64,8192,64,adaptive,32,18.000,23.000,28.000,9.000,11.000,13.000,180,900,1700,3000,700,1000,200,3000,120,90,70,70,6,6
"#,
    );
    ...
    assert!(stdout.contains(
        "policy_recommend clients=2 relay_batch_frames=64 recommended=adaptive verdict=adaptive_better reason=adaptive_not_worse"
    ));
    assert!(stdout.contains(
        "policy_recommend clients=4 relay_batch_frames=64 recommended=fixed verdict=fixed_better reason=fixed_not_worse"
    ));
    assert!(stdout.contains(
        "policy_recommend clients=6 relay_batch_frames=64 recommended=fixed verdict=same_effective reason=same_effective_keep_default"
    ));
    assert!(stdout.contains(
        "policy_recommend clients=8 relay_batch_frames=64 recommended=manual_review verdict=mixed reason=metric_tradeoff"
    ));
}
