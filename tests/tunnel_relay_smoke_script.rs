...
fn tunnel_relay_smoke_script_runs_runtime_relay_test() {
    ...
    assert!(script.contains("KTP_SMOKE_CODEC_RUNS"));
    assert!(script.contains("KTP_SMOKE_CODEC_FRAMES"));
    assert!(script.contains("KTP_SMOKE_CODEC_PAYLOAD_BYTES"));
    assert!(script.contains("KTP_SMOKE_CODEC_CHUNK_FRAMES"));
    assert!(script.contains("cargo run --bin ktp-codec-bench"));
    assert!(script.contains("--mode stream"));
    assert!(script.contains("--mode crypto"));
