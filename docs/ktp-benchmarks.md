...
KTP TCP carrier, `ktp_aead` crypto, token auth preface, relay batch policy, and
adaptive thresholds, then verifies that `tunnel data diagnostics` lines include
runtime wait, lifetime outbound queue dwell, recent outbound queue dwell,
socket batch-read/write counters, and effective write batch limit max/min/last
fields.
...
the observation window. For adaptive scheduling, read `max` as the largest cap
observed, `min` as the lowest pressure-reduced cap, and `last` as the most
...
- Production tunnel-data diagnostics also expose `socket_write_batches`,
  `socket_write_frames`, `socket_write_max_batch_frames`,
  `socket_write_batch_limit_max`, `socket_write_batch_limit_min`, and
  `socket_write_batch_limit_last`. This lets live canaries prove that runtime
  frames reached the carrier through the batched send path, and records both
...
RDP-like local-backend KTP smoke:

- Agent commit: `e61bdd7`
- Backend commit: `334a82c`
- Smoke driver:
  - `KELICLOUD_TUNNEL_ECHO_ROUNDS=8`
  ...
  - `KTP_LIVE_CANARY_MIN_MAX_BATCH_FRAMES=2`
- Run directory: `/tmp/kelicloud-agent-rs-ktp-rdp-e61bdd7-211750`
- Tunnel echo evidence:
  `/tmp/kelicloud-agent-rs-ktp-rdp-e61bdd7-211750/logs/tunnel-echo.evidence.md`
- KTP evidence:
  `/tmp/kelicloud-agent-rs-ktp-rdp-e61bdd7-211750/logs/ktp-live-canary.evidence.md`

Tunnel echo RTT evidence:

| Profile | Rounds | Total Payload | RTT p50 | RTT p95 | RTT p99 | RTT Max |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| rdp-like | 8 | 9920 B | 13666 us | 15121 us | 15121 us | 15121 us |
...
- The `Local Backend Smoke` GitHub Actions workflow now runs the same smoke as
  ...
  --require-ktp-tunnel-rtt --require-ktp-rdp-like-rtt
  --max-ktp-rdp-like-rtt-p95-micros 250000
  --max-ktp-rdp-like-client-p95-spread-micros 250000`, and uploads
  ...
  The 250 ms p95/spread thresholds are broad regression gates for local CI
  loopback forwarding, not production latency claims.
...
- Newer canary evidence also requires positive `socket_write_batches`,
  `socket_write_frames`, `socket_write_max_batch_frames`,
  `socket_write_batch_limit_max`, `socket_write_batch_limit_min`, and
  `socket_write_batch_limit_last`, so the same live smoke proves runtime frames
  used the batched carrier send path and records the selected write batch caps.
