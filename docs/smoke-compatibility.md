...
- [ ] GitHub release exists with `kelicloud-agent-rs-linux-amd64`,
  `kelicloud-agent-rs-linux-arm64`, and `kelicloud-agent-rs-linux-armv7`
  assets.
- [ ] A fresh Linux host installs from the panel-compatible auto-discovery
  command:
...
- [ ] The installed service appears online in the panel with sane CPU, RAM,
...
- [ ] An admin WebSSH terminal opens, accepts input, and closes cleanly.
- [ ] `systemctl restart kelicloud-agent-rs` reconnects without manual token
  repair.
- [ ] Re-running the install command with `--install-version <tag>` upgrades or
  pins the expected release asset.
- [ ] `sudo ./install.sh uninstall` removes the systemd service, binary, config,
  and auto-discovery cache when `--keep-config` is not used.
- [ ] Rollback path is verified by uninstalling the Rust service and installing
  the Go agent command generated by the panel.
