...
DIRECTIONS="${KTP_CARRIER_MATRIX_DIRECTIONS:-client-to-relay client-to-relay-batch-write relay-to-client-batch-read}"
...
write_csv_row() {
  ...
  read_batch_reused="$(metric_value "${output}" read_batch_reused)"
  if [[ -z "${read_batch_reused}" ]]; then
    read_batch_reused="0"
  fi
...
for direction in ${DIRECTIONS}; do
  if [[ "${direction}" != "client-to-relay" && "${direction}" != "client-to-relay-batch-write" && "${direction}" != "relay-to-client-batch-read" ]]; then
